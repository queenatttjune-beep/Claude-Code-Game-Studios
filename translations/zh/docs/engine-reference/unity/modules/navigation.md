...
3. 配置设置：
   - **Agent Radius**：agent 的宽度（默认 0.5m）
   - **Agent Height**：agent 的高度（默认 2m）
   - **Max Slope**：最大可行走坡度（默认 45°）
   - **Step Height**：最大可跨步高度（默认 0.4m）
...
// Carve：在 NavMesh 中创建孔洞（agent 避开）
// Don't Carve：agent 穿过（本地回避）
```
...
- **限制障碍物回避质量**：对远处 agent 使用 `LowQualityObstacleAvoidance`
- **更新频率**：如果目标未移动，不要每帧都调用 `SetDestination()`
- **区域掩码**：限制可行走区域以减少寻路搜索空间
- **NavMesh 瓦片**：大型世界使用瓦片化 NavMesh（NavMeshComponents 包）
