...
LLM 的训练数据可能只覆盖到 Unity ~2022 LTS（2022.3）。整个 Unity 6 发布系列（原 Unity 2023 Tech Stream）引入了模型不知道的重大变化。在建议 Unity API 调用之前，始终交叉参考本目录。
...
| 6.3 LTS | 2025 年 12 月 | 高 | 自 6.0 以来首个 LTS、生产就绪的 DOTS、增强的图形功能 |

## 从 2022 LTS 到 Unity 6.3 LTS 的主要变更

### 破坏性变更
- **Entities/DOTS**：Entities 1.0+ 中的主要 API 全面改造、ECS 模式完全重新设计
- **输入系统**：传统输入管理器已弃用、新输入系统成为默认
...
- **UI Toolkit**：生产就绪的运行时 UI（新项目替代 UGUI）
- **异步资源加载**：改进的 Addressables 性能
- **Web**：WebGPU 支持

### 已弃用的系统
- **传统输入管理器**：使用新的 Input System 包
- **传统粒子系统**：使用 Visual Effect Graph
- **UGUI**：仍受支持，但新项目推荐 UI Toolkit
- **旧 ECS（GameObjectEntity）**：被现代 DOTS/Entities 替代
