...
无。 `/sprint-status` is a read-only reporting skill; no gates are invoked.

---

## 测试用例

### 用例 1: Happy Path — Mixed sprint, AT RISK with named blocker
...
### 用例 2: All Stories Complete — Sprint COMPLETE verdict
...
### 用例 3: No Active Sprint File — Guidance to run /sprint-plan
...
### 用例 4: Edge Case — Stale In Progress Story (flagged)
...
### 用例 5: Gate Compliance — Read-only; no gate invocation
