...
invoked. The skill does not write without user approval. 裁决： NO FLAKINESS,
...
无。 Flakiness detection is an advisory quality skill for the QA lead; no gates
are invoked.
...
### 用例 1: Happy Path — Clean test history, no flakiness
...
### 用例 2: Suspect Tests Found — Test fails intermittently in history
...
### 用例 3: Source Pattern — Random number used without seed
...
### 用例 4: No Test History — Source-only analysis with common patterns
...
### 用例 5: Gate Compliance — No gate; flakiness report is advisory
