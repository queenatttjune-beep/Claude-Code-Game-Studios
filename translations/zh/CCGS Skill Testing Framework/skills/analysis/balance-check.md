...
a balance report but asks "May I write" before doing so. 裁决： BALANCED,
...
无。 Balance check is a read-only analysis skill; no gates are invoked.

---

## 测试用例

### 用例 1: Happy Path — All balance values within formula tolerances
...
### 用例 2: Out of Balance — Player damage 40% above formula target
...
### 用例 3: No GDD Formulas — Cannot validate, guidance given
...
### 用例 4: Orphan Reference — Balance file references an undefined stat
...
### 用例 5: Gate Compliance — Read-only; no gate; optional report requires approval
