...
blocking gate. 裁决： HOTFIX COMPLETE (fix applied, smoke check passed, merged)
...
无。 Hotfixes are time-critical. Director review may follow separately as a
...
### 用例 1: Happy Path — Critical crash bug fixed, smoke check passes
...
**输入：** `/hotfix` (user describes the crash and affected file)
...
### 用例 2: Smoke Check Fails — HOTFIX BLOCKED
...
### 用例 3: Fix to Already-Released Build — Version tag noted, patch bump prompted
...
### 用例 5: Director Gate Check — No gate; hotfixes are time-critical
