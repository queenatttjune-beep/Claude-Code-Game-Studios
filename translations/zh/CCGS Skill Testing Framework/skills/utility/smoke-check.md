...
裁决： PASS (tests pass, all smoke checks pass, no missing test evidence),
...
无。 `/smoke-check` is a pre-QA utility skill. No director gates apply.

---

## 测试用例

### 用例 1: Happy Path — Automated tests pass, manual items confirmed, PASS
...
### 用例 2: Failure Path — Automated test fails, FAIL verdict
...
### 用例 3: Manual Confirmation — AskUserQuestion used, PASS WITH WARNINGS
...
### 用例 4: No Test Directory — Skill stops with guidance
...
### 用例 5: Director Gate Check — No gate; smoke-check is a QA pre-check utility
