...
### 用例 1: Happy Path — All Concept artifacts present, advancing to Systems Design
...
### 用例 2: Failure Path — Missing required artifacts for Concept → Systems Design
...
### 用例 3: No Argument — Auto-detect current stage
...
### 用例 4: Edge Case — Manual check items flagged correctly
...
### 用例 5: Director Gate — lean vs full vs solo mode
...
**输入：** `/gate-check systems-design` (with full mode active)
...
**输入：** `/gate-check systems-design` (with solo mode active)
